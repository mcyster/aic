...
One `ModelDriver::invoke` call represents one provider/model invocation. Its outer future establishes the invocation and returns a stream; the stream then yields a completed `ModelDriverEvent` or a typed error. `ModelDriverEvent::Model` contains successful semantic `ModelEvent` output, while `ModelDriverEvent::Problem` contains a semantic `ModelIssue`; each may carry opaque `ModelData` defined by the driver that produced it. The caller knows which driver it invoked, maps the event to its canonical conversation event kind, adds the driver's typed provider/model `ModelSource` and envelope metadata, and appends or publishes the result. New conversation state exists only after those events are appended.
