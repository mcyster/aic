...
One `ModelDriver::invoke` call represents one provider/model invocation. Its outer future establishes the invocation and returns a stream; the stream then yields a completed `ModelDriverOutput` or a typed error. `ModelDriverOutput::Event` contains successful semantic `ModelEvent` output, while `ModelDriverOutput::Issue` contains a semantic `ModelIssue`. The caller knows which driver it invoked, maps the output to its canonical conversation event, adds the driver's typed provider/model `ModelSource` and envelope metadata, and appends or publishes the result. New conversation state exists only after those events are appended.

`ConversationEventKind::Problem` contains one authoritative `ModelProblem` and its `ModelSource`. `ModelProblem::Issue` records a meaningful limitation or unsuccessful outcome understood by the driver, such as a refusal or exceeded context limit. `ModelProblem::Invocation` records a sanitized operational failure. `ModelDriverError` remains the detailed Rust control-flow error; the turn service converts it into a durable invocation problem before returning it. The problem itself owns its message and retryability semantics; the conversation event does not duplicate message or severity fields.
...
The first `ModelDriver` uses OpenAI's Responses API through a narrow asynchronous HTTP/SSE adapter. One invocation uses one REST request and one SSE response stream. The driver owns a stable source containing provider `openai` and its configured model, translates the complete semantic conversation into provider input, keeps protocol events and raw text deltas private, aggregates provider deltas, and yields completed semantic outputs. Consuming several outputs does not make several model requests. Final output becomes an `AssistantResponse`; exposed reasoning and reasoning summaries become `ModelCommunication`s with their declared importance; refusals and recognized context-limit responses become model issues. Known model-associated concepts have explicit contracts, while communication extensions and `ModelIssue::Other` extension data retain auxiliary driver-specific enrichment without replacing portable fields. Every model-associated `ConversationEvent` records its source, but full invocation configuration is not repeated on each event. Raw provider events, raw provider error bodies, and OpenAI response IDs are not persisted as semantic facts. Every invocation reconstructs provider input from the Conversation Log, so continuation does not depend on OpenAI history and another driver can continue the conversation.
