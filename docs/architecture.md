...
Individual `ConversationEvent`s are published to an event bus; `Conversation` itself is not. A `ModelDriver` receives an immutable reference to the reconstructed `Conversation` and returns typed model-produced `ModelEvent`s. The caller knows which driver it invoked and combines each returned event with that driver's typed provider/model `ModelSource` before assigning canonical envelope metadata and appending or publishing the resulting `ConversationEvent`. New conversation state exists only after those events are appended.
...
The first `ModelDriver` uses OpenAI's Responses API through a narrow HTTP/SSE adapter. It owns a stable source containing provider `openai` and its configured model, translates the complete semantic conversation into provider input, consumes streaming protocol events internally, and returns typed `ModelEvent`s. Final output becomes an important `AssistantResponse`; exposed reasoning and reasoning summaries become `ModelCommunication`s with their declared importance. Known model-produced concepts have explicit contracts, while communication extensions retain auxiliary driver-specific enrichment without replacing portable fields. Every model-generated `ConversationEvent` records its source, but full invocation configuration is not repeated on each event. Raw provider events and OpenAI response IDs are not persisted. Every invocation reconstructs provider input from the Conversation Log, so continuation does not depend on OpenAI history and another driver can continue the conversation.
