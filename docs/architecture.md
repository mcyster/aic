...
One `ModelDriver::invoke` call represents one provider/model invocation. Its outer future establishes the invocation and returns a stream; the stream then yields completed semantic `ModelEvent`s or a typed error. The caller knows which driver it invoked and combines each yielded event with that driver's typed provider/model `ModelSource` before assigning canonical envelope metadata and appending or publishing the resulting `ConversationEvent`. A `ModelEvent` may be an assistant response, a communication, or a `ModelProblem`. New conversation state exists only after those events are appended.

`ModelProblem::Issue` records a meaningful limitation or unsuccessful outcome understood by the driver, such as a refusal or exceeded context limit. `ModelProblem::Invocation` records a sanitized operational failure. `ModelDriverError` remains the detailed Rust control-flow error; the turn service converts it into a durable invocation problem before returning it. Both problem categories remain under `ConversationEventKind::Model` because they are facts associated with the selected model invocation and carry its `ModelSource`.
