...
Semantically important structured concepts remain structured in the portable representation. For example, a tool request retains its portable call ID, tool name, and arguments even when it also carries a provider-specific call ID. Driver-specific enrichment lives in `ModelDetails` on model-associated event kinds. `ModelDetails` keeps the `ModelSource` and optional `ModelData` together, so the source identifies which driver may interpret the opaque data. Raw provider transport events do not become semantic conversation events merely because they are provider-specific.
